...
import { $, escapeHtml, formatDate, initialsForName, money } from './utils.js';
...
export function renderApp(store, openCategoryId, actions) {
  ...
  $('#periodStatus').textContent = store.periodIsCurrent() ? 'Période actuelle' : 'Historique';
  $('#periodSelector').innerHTML = `${formatDate(period.start, { day: 'numeric', month: 'long' })} <span>⌄</span>`;
  $('#periodDates').textContent = `${formatDate(period.start)} au ${formatDate(period.end)}`;
  ...
  store.state.categories.forEach((category, index) => {
    list.appendChild(createCategoryCard(store, category, index, openCategoryId, actions));
  });
...
function createCategoryCard(store, category, index, openCategoryId, actions) {
  ...
  const transactionCount = line.expenses.length + (Array.isArray(line.extras) ? line.extras.length : line.extra ? 1 : 0);
  ...
  card.querySelector('.category-header').addEventListener('click', () => actions.toggleCategory(category.id));
  card.querySelector('[data-action="expense"]').addEventListener('click', () => actions.openTransaction(category.id, 'expense'));
  card.querySelector('[data-action="extra"]').addEventListener('click', () => actions.openTransaction(category.id, 'extra'));
  card.querySelectorAll('[data-expense]').forEach((button) => button.addEventListener('click', () => actions.removeExpense(category.id, button.dataset.expense)));
  card.querySelectorAll('[data-extra]').forEach((button) => button.addEventListener('click', () => actions.removeExtra(category.id, button.dataset.extra)));
  return card;
}
...
function transactionMarkup(line, periodStartKey) {
  const legacyExtras = !Array.isArray(line.extras) && line.extra
    ? [{ id: 'legacy-extra', name: 'Extra', amount: line.extra, date: periodStartKey, legacy: true }]
    : [];
  ...
  if (!transactions.length) return '<p class="empty-expenses">Aucune transaction ici pour le moment.</p>';
  return transactions.map((transaction) => `<div class="expense-row"><div><strong>${escapeHtml(transaction.name)}</strong><div class="expense-meta">${transaction.type === 'extra' ? 'Extra · ' : ''}${formatDate(transaction.date, { day: 'numeric', month: 'short', year: 'numeric' })}</div></div><div class="expense-actions"><span class="expense-amount ${transaction.type === 'extra' ? 'extra-amount' : ''}">${transaction.type === 'extra' ? '+' : '−'} ${money(transaction.amount)}</span>${transaction.legacy ? '' : `<button class="delete-expense" type="button" data-${transaction.type}="${transaction.id}" aria-label="Supprimer ${escapeHtml(transaction.name)}">×</button>`}</div></div>`).join('');
}

function renderInsight(remaining, spent, total) {
  const ratio = total ? spent / total : 0;
  $('#insightTitle').textContent = remaining < 0 ? 'On ajuste doucement' : ratio > .75 ? 'La période avance bien' : 'Petit pas, grand équilibre';
  $('#insightText').textContent = remaining < 0 ? 'Une enveloppe est dépassée. Tu peux ajouter un extra pour rééquilibrer.' : ratio > .75 ? 'Tu as utilisé la majorité du budget de cette période.' : 'Chaque dépense ajoutée garde ton budget à jour.';
}
